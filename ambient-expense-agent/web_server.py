...
async def run_cloud_agent_background(transaction_id: str, query: str, user: str, role: str):
    ...
    try:
        ...
        # Corrected: Create a standard session context dynamically generated by the ADK framework
        ...
        async for event in runner.run_async(
            ...
            if event.long_running_tool_ids and "manual_approval" in event.long_running_tool_ids:
                ...
                if event.content and event.content.parts:
                    fc = event.content.parts.function_call
                    if fc and fc.args:
                        interrupt_msg = fc.args.get("message", interrupt_msg)
                
                # Corrected: Pass the exact session.id token back to the client interface layout handler
